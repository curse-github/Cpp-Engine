...
int main(int argc, char** argv) {
	...
	playerShader->setTexture("_texture", &instanceUnlitTex, 0);
	...
	// map and minimap
	sceneRenderers.push_back(new SpriteRenderer(engine, backgroundShader, fullMapSize/2.0f, fullMapSize, 0.0f));// background
	...
	for(unsigned int i=0; i<instanceData.size(); i++) {
		...
		SpriteRenderer* tmp = new SpriteRenderer(engine, instanceUnlitShader, gridToWorld(Vector2((float)dat[0]+0.5f, (float)dat[1]+0.5f)), Vector2(mapScale, mapScale), 0.0f);
		tmp->zIndex=1;
		machineRenderers.push_back(tmp);
	}
